...
public interface JournalpostRepository extends EinnsynRepository<Journalpost, Long> {
  ...
  @Query(
      "SELECT j FROM Journalpost j, Saksmappe s WHERE j.saksmappe = s AND s.id = ?1 ORDER BY j.id DESC")
  public Page<Journalpost> findBySaksmappeIdOrderByIdDesc(String saksmappeId, Pageable pageable);

  @Query(
      "SELECT j FROM Journalpost j, Saksmappe s WHERE j.saksmappe = s AND s.id = ?1 AND j.id > ?2 ORDER BY j.id DESC")
