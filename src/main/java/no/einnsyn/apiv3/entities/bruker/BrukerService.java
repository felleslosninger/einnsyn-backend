...
public class BrukerService extends EinnsynObjectService<Bruker, BrukerJSON> {
  ...
  @Lazy @Resource private InnsynskravService innsynskravService;

  @Resource private InnsynskravDelService innsynskravDelService;
  ...
  @Override
  @Transactional
  public BrukerJSON toJSON(
      Bruker bruker, BrukerJSON json, Set<String> expandPaths, String currentPath) {
    ...
    if (innsynskravList != null) {
      innsynskravList.forEach(
          innsynskrav -> {
            var expandableField =
                innsynskravService.maybeExpand(
                    innsynskrav, "innsynskrav", expandPaths, currentPath);
  ...
  @Transactional
  @SuppressWarnings("java:S6809") // this.toJSON() is OK since we're already in a transaction
  public BrukerJSON updatePasswordWithOldPassword(
      Bruker bruker, String oldPassword, String password) throws UnauthorizedException {
