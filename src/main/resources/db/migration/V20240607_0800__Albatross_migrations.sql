-- ID generator
create
or replace function einnsyn_id(prefix text) returns text as $ $ declare uuid uuid;

suffix text;

begin if (prefix is null)
or not (prefix ~ '^[a-z]{0,63}$') then raise exception 'prefix must match the regular expression [a-z]{0,63}';
...
CREATE
OR REPLACE FUNCTION add_foreign_key_if_not_exists(
  p_table_name TEXT,
  p_column_name TEXT,
  p_reference_table TEXT,
  p_reference_column TEXT
) RETURNS VOID AS $ $ DECLARE v_constraint_name TEXT;

BEGIN -- Generate the constraint name by concatenating the table name and column name
...
IF NOT EXISTS (
  ...
  FROM
    pg_constraint
  WHERE
    conname = v_constraint_name
) THEN -- Construct and execute the ALTER TABLE statement
EXECUTE format(
 ...
 */
CREATE
OR REPLACE FUNCTION enrich_legacy_journalenhet() RETURNS TRIGGER AS $ $ BEGIN IF NEW.journalenhet__id IS NULL
AND NEW.virksomhet_iri IS NOT NULL THEN
SELECT
  _id INTO NEW.journalenhet__id
FROM
  enhet
WHERE
  iri = NEW.virksomhet_iri;
...
ALTER TABLE
  IF EXISTS enhet
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('enh'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ default now(),
ADD
  COLUMN IF NOT EXISTS _updated TIMESTAMPTZ default now(),
ADD
  COLUMN IF NOT EXISTS lock_version BIGINT NOT NULL DEFAULT 1,
ALTER COLUMN
  opprettet_dato
SET
  DEFAULT now(),
...
SET
  _external_id = iri
WHERE
  _external_id IS NULL;

-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_enhet() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.iri IS NOT NULL
AND NEW.iri != NEW._id THEN NEW._external_id := NEW.iri;
...
CREATE TRIGGER enrich_legacy_enhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON enhet FOR EACH ROW EXECUTE FUNCTION enrich_legacy_enhet();

/* Arkiv */
CREATE TABLE IF NOT EXISTS arkiv(_id TEXT DEFAULT einnsyn_id('ark'));

ALTER TABLE
  IF EXISTS arkiv
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('ark'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
ADD
  COLUMN IF NOT EXISTS _updated TIMESTAMPTZ DEFAULT now(),
ADD
  COLUMN IF NOT EXISTS system_id TEXT,
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
  /* This is a legacy field, but the object should inherit from ArkivBase: */
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT;

SELECT
  add_foreign_key_if_not_exists('arkiv', 'journalenhet__id', 'enhet', '_id');
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_arkiv() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.arkiv_iri IS NOT NULL
AND NEW.arkiv_iri != NEW._id THEN NEW._external_id = NEW.arkiv_iri;
...
CREATE TRIGGER enrich_legacy_arkiv_trigger BEFORE
INSERT
  OR
UPDATE
  ON arkiv FOR EACH ROW EXECUTE FUNCTION enrich_legacy_arkiv();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_arkiv_journalenhet_trigger ON arkiv;

CREATE TRIGGER enrich_legacy_arkiv_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON arkiv FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Arkivdel */
CREATE TABLE IF NOT EXISTS arkivdel(_id TEXT DEFAULT einnsyn_id('arkd'));

ALTER TABLE
  IF EXISTS arkivdel
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('arkd'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
SELECT
  add_foreign_key_if_not_exists('arkivdel', 'journalenhet__id', 'enhet', '_id');
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_arkivdel() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.arkivdel_iri IS NOT NULL
AND NEW.arkivdel_iri != NEW._id THEN NEW._external_id = NEW.arkivdel_iri;
...
CREATE TRIGGER enrich_legacy_arkivdel_trigger BEFORE
INSERT
  OR
UPDATE
  ON arkivdel FOR EACH ROW EXECUTE FUNCTION enrich_legacy_arkivdel();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_arkivdel_journalenhet_trigger ON arkivdel;

CREATE TRIGGER enrich_legacy_arkivdel_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON arkivdel FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Klassifikasjonssystem */
CREATE TABLE IF NOT EXISTS klassifikasjonssystem(_id TEXT DEFAULT einnsyn_id('ksys'));

ALTER TABLE
  IF EXISTS klassifikasjonssystem
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('ksys'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS lock_version INT DEFAULT 0,
  /* This is a legacy field, but the object should inherit from ArkivBase: */
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT,
ADD
  COLUMN IF NOT EXISTS arkivdel__id TEXT,
ADD
  COLUMN IF NOT EXISTS tittel TEXT;
...
/* Klasse */
CREATE TABLE IF NOT EXISTS klasse(_id TEXT DEFAULT einnsyn_id('kla'));

ALTER TABLE
  IF EXISTS klasse
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('kla'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT,
ADD
  COLUMN IF NOT EXISTS klassifikasjonssystem__id TEXT;

SELECT
  add_foreign_key_if_not_exists('klasse', 'journalenhet__id', 'enhet', '_id');

SELECT
  add_foreign_key_if_not_exists(
    'klasse',
    'klassifikasjonssystem__id',
    'klassifikasjonssystem',
    '_id'
  );
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_klasse() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.klasse_iri IS NOT NULL
AND NEW.klasse_iri != NEW._id THEN NEW._external_id = NEW.klasse_iri;
...
CREATE TRIGGER enrich_legacy_klasse_trigger BEFORE
INSERT
  OR
UPDATE
  ON klasse FOR EACH ROW EXECUTE FUNCTION enrich_legacy_klasse();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_klasse_journalenhet_trigger ON klasse;

CREATE TRIGGER enrich_legacy_klasse_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON klasse FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Saksmappe */
ALTER TABLE
  IF EXISTS saksmappe
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('sm'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ default now(),
...
ADD
  COLUMN IF NOT EXISTS system_id TEXT,
ADD
  COLUMN IF NOT EXISTS administrativ_enhet TEXT,
ADD
  COLUMN IF NOT EXISTS administrativ_enhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS last_indexed TIMESTAMPTZ;

SELECT
  add_foreign_key_if_not_exists('saksmappe', 'journalenhet__id', 'enhet', '_id');
...
CREATE INDEX IF NOT EXISTS saksmappe__created_null_idx ON saksmappe (_created)
WHERE
  _created IS NULL;

CREATE INDEX IF NOT EXISTS saksmappe__updated_null_idx ON saksmappe (_updated)
WHERE
  _updated IS NULL;

-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_saksmappe() RETURNS TRIGGER AS $ $ BEGIN -- Set _external_id to saksmappe_iri for old import
IF NEW._external_id IS NULL
AND NEW.saksmappe_iri IS NOT NULL
AND NEW.saksmappe_iri != NEW._id THEN NEW._external_id := NEW.saksmappe_iri;

END IF;

-- Look up adminiistrativ_enhet__id for old import
IF TG_OP = 'UPDATE'
AND NEW.arkivskaper IS DISTINCT
FROM
  OLD.arkivskaper THEN
SELECT
  _id INTO NEW.administrativ_enhet__id
FROM
  enhet
WHERE
  iri = NEW.arkivskaper;
...
CREATE TRIGGER enrich_legacy_saksmappe_trigger BEFORE
INSERT
  OR
UPDATE
  ON saksmappe FOR EACH ROW EXECUTE FUNCTION enrich_legacy_saksmappe();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_saksmappe_journalenhet_trigger ON saksmappe;

CREATE TRIGGER enrich_legacy_saksmappe_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON saksmappe FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Journalpost */
ALTER TABLE
  IF EXISTS journalpost
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('jp'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ default now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS saksbehandler TEXT,
ADD
  COLUMN IF NOT EXISTS beskrivelse TEXT,
ADD
  COLUMN IF NOT EXISTS last_indexed TIMESTAMPTZ;
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_journalpost() RETURNS TRIGGER AS $ $ BEGIN -- Set _external_id to journalpost_iri for old import
IF NEW._external_id IS NULL
AND NEW.journalpost_iri IS NOT NULL
AND NEW.journalpost_iri != NEW._id THEN NEW._external_id := NEW.journalpost_iri;

END IF;

-- Set saksmappe_iri for new import
IF NEW.saksmappe_iri IS NULL
AND NEW.saksmappe_id IS NOT NULL THEN
SELECT
  _external_id INTO NEW.saksmappe_iri
FROM
  saksmappe
WHERE
  saksmappe_id = NEW.saksmappe_id;

END IF;

-- Set administrativ_enhet__id from arkivskaper for old import
IF (
  NEW.administrativ_enhet__id IS NULL
  AND NEW.arkivskaper IS NOT NULL
)
OR (
  TG_OP = 'UPDATE'
  AND NEW.arkivskaper IS DISTINCT
  FROM
    OLD.arkivskaper
    AND NEW.administrativ_enhet__id IS NOT DISTINCT
  FROM
    OLD.administrativ_enhet__id
) THEN
SELECT
  _id INTO NEW.administrativ_enhet__id
FROM
...
CREATE TRIGGER enrich_legacy_journalpost_trigger BEFORE
INSERT
  OR
UPDATE
  ON journalpost FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalpost();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_journalpost_journalenhet_trigger ON journalpost;

CREATE TRIGGER enrich_legacy_journalpost_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON journalpost FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Skjerming */
ALTER TABLE
  IF EXISTS skjerming
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('skj'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
SELECT
  add_foreign_key_if_not_exists('skjerming', 'journalenhet__id', 'enhet', '_id');
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_skjerming() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.skjerming_iri IS NOT NULL
AND NEW.skjerming_iri != NEW._id THEN NEW._external_id := NEW.skjerming_iri;
...
CREATE TRIGGER enrich_legacy_skjerming_trigger BEFORE
INSERT
  OR
UPDATE
  ON skjerming FOR EACH ROW EXECUTE FUNCTION enrich_legacy_skjerming();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_skjerming_journalenhet_trigger ON skjerming;

CREATE TRIGGER enrich_legacy_skjerming_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON skjerming FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Moetesaksbeskrivelse */
CREATE TABLE IF NOT EXISTS moetesaksbeskrivelse(_id TEXT DEFAULT einnsyn_id('msb'));

ALTER TABLE
  IF EXISTS moetesaksbeskrivelse
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('msb'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS tekst_innhold TEXT,
ADD
  COLUMN IF NOT EXISTS tekst_format TEXT,
  /* This is a legacy field, but the object should inherit from ArkivBase: */
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT;
...
CREATE TRIGGER enrich_legacy_moetesaksbeskrivelse_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON moetesaksbeskrivelse FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Møtedokumentregistrering */
CREATE TABLE IF NOT EXISTS møtedokumentregistrering(_id TEXT DEFAULT einnsyn_id('mdok'));

ALTER TABLE
  IF EXISTS møtedokumentregistrering
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('mdok'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS beskrivelse TEXT,
ALTER COLUMN
  møtemappe_id DROP NOT NULL,
  /* Eases insertion from parent møtemappe */
  /* This is a legacy field, but the object should inherit from Registrering: */
ADD
  COLUMN IF NOT EXISTS arkivskaper TEXT,
  /* This is a legacy field, but the object should inherit from ArkivBase: */
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT;
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_moetedokumentregistrering() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.møtedokumentregistrering_iri IS NOT NULL
AND NEW.møtedokumentregistrering_iri != NEW._id THEN NEW._external_id := NEW.møtedokumentregistrering_iri;
...
CREATE TRIGGER enrich_legacy_moetedokumentregistrering_trigger BEFORE
INSERT
  OR
UPDATE
  ON møtedokumentregistrering FOR EACH ROW EXECUTE FUNCTION enrich_legacy_moetedokumentregistrering();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_moetedokumentregistrering_journalenhet_trigger ON møtedokumentregistrering;

CREATE TRIGGER enrich_legacy_moetedokumentregistrering_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON møtedokumentregistrering FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Korrespondansepart */
ALTER TABLE
  IF EXISTS korrespondansepart
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('kp'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS moetedokument__id TEXT,
ADD
  COLUMN IF NOT EXISTS moetesak__id TEXT,
ADD
  COLUMN IF NOT EXISTS er_behandlingsansvarlig BOOLEAN DEFAULT FALSE,
ALTER COLUMN
  journalpost_id DROP NOT NULL,
  -- Korrespondansepart could also be tied to moetesak / moetedokument
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_korrespondansepart() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.korrespondansepart_iri IS NOT NULL
AND NEW.korrespondansepart_iri != NEW._id THEN NEW._external_id := NEW.korrespondansepart_iri;
...
CREATE TRIGGER enrich_legacy_korrespondansepart_trigger BEFORE
INSERT
  OR
UPDATE
  ON korrespondansepart FOR EACH ROW EXECUTE FUNCTION enrich_legacy_korrespondansepart();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_korrespondansepart_journalenhet_trigger ON korrespondansepart;

CREATE TRIGGER enrich_legacy_korrespondansepart_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON korrespondansepart FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Dokumentbeskrivelse */
ALTER TABLE
  IF EXISTS dokumentbeskrivelse
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('db'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_dokumentbeskrivelse() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.dokumentbeskrivelse_iri IS NOT NULL
AND NEW.dokumentbeskrivelse_iri != NEW._id THEN NEW._external_id := NEW.dokumentbeskrivelse_iri;
...
CREATE TRIGGER enrich_legacy_dokumentbeskrivelse_trigger BEFORE
INSERT
  OR
UPDATE
  ON dokumentbeskrivelse FOR EACH ROW EXECUTE FUNCTION enrich_legacy_dokumentbeskrivelse();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_dokumentbeskrivelse_journalenhet_trigger ON dokumentbeskrivelse;

CREATE TRIGGER enrich_legacy_dokumentbeskrivelse_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON dokumentbeskrivelse FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Dokumentobjekt */
ALTER TABLE
  IF EXISTS dokumentobjekt
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('do'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_dokumentobjekt() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.dokumentobjekt_iri IS NOT NULL
AND NEW.dokumentobjekt_iri != NEW._id THEN NEW._external_id := NEW.dokumentobjekt_iri;
...
CREATE TRIGGER enrich_legacy_dokumentobjekt_trigger BEFORE
INSERT
  OR
UPDATE
  ON dokumentobjekt FOR EACH ROW EXECUTE FUNCTION enrich_legacy_dokumentobjekt();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_dokumentobjekt_journalenhet_trigger ON dokumentobjekt;

CREATE TRIGGER enrich_legacy_dokumentobjekt_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON dokumentobjekt FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Moetemappe */
CREATE TABLE IF NOT EXISTS møtemappe(_id TEXT DEFAULT einnsyn_id('mm'));

ALTER TABLE
  IF EXISTS møtemappe
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('mm'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS utvalg__id TEXT,
ADD
  COLUMN IF NOT EXISTS last_indexed TIMESTAMPTZ,
ADD
  COLUMN IF NOT EXISTS referanse_forrige_moete__id TEXT,
ADD
  COLUMN IF NOT EXISTS referanse_neste_moete__id TEXT,
  /* This is a legacy field, but the object should inherit from ArkivBase: */
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT;
...
SELECT
  add_foreign_key_if_not_exists('møtemappe', 'journalenhet__id', 'enhet', '_id');

SELECT
  add_foreign_key_if_not_exists('møtemappe', 'utvalg__id', 'enhet', '_id');

SELECT
  add_foreign_key_if_not_exists(
    'møtemappe',
    'referanse_forrige_moete__id',
    'møtemappe',
    '_id'
  );
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_moetemappe() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.møtemappe_iri IS NOT NULL
AND NEW.møtemappe_iri != NEW._id THEN NEW._external_id := NEW.møtemappe_iri;
...
CREATE TRIGGER enrich_legacy_moetemappe_trigger BEFORE
INSERT
  OR
UPDATE
  ON møtemappe FOR EACH ROW EXECUTE FUNCTION enrich_legacy_moetemappe();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_moetemappe_journalenhet_trigger ON møtemappe;

CREATE TRIGGER enrich_legacy_moetemappe_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON møtemappe FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();

/* Behandlingsprotokoll */
CREATE TABLE IF NOT EXISTS behandlingsprotokoll(_id TEXT DEFAULT einnsyn_id('bp'));

ALTER TABLE
  IF EXISTS behandlingsprotokoll
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('bp'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
/* Vedtak */
CREATE TABLE IF NOT EXISTS vedtak(_id TEXT DEFAULT einnsyn_id('ved'));

ALTER TABLE
  IF EXISTS vedtak
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('ved'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS vedtakstekst__id TEXT,
ADD
  COLUMN IF NOT EXISTS behandlingsprotokoll__id TEXT,
ADD
  COLUMN IF NOT EXISTS dato DATE,
  /* This is a legacy field, but the object should inherit from ArkivBase: */
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT;

SELECT
  add_foreign_key_if_not_exists('vedtak', 'journalenhet__id', 'enhet', '_id');
...
/* Utredning */
CREATE TABLE IF NOT EXISTS utredning(_id TEXT DEFAULT einnsyn_id('utr'));

ALTER TABLE
  IF EXISTS utredning
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('utr'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS saksbeskrivelse__id TEXT,
ADD
  COLUMN IF NOT EXISTS innstilling__id TEXT,
  /* This is a legacy field, but the object should inherit from ArkivBase: */
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT;

SELECT
  add_foreign_key_if_not_exists('utredning', 'journalenhet__id', 'enhet', '_id');
...
CREATE TABLE IF NOT EXISTS møtesaksregistrering(_id TEXT DEFAULT einnsyn_id('ms'));

ALTER TABLE
  IF EXISTS møtesaksregistrering
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('ms'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS last_indexed TIMESTAMPTZ,
ADD
  COLUMN IF NOT EXISTS utredning__id TEXT,
ADD
  COLUMN IF NOT EXISTS innstilling__id TEXT,
ADD
  COLUMN IF NOT EXISTS vedtak__id TEXT,
ADD
  COLUMN IF NOT EXISTS journalpost__id TEXT,
ADD
  COLUMN IF NOT EXISTS beskrivelse TEXT,
ALTER COLUMN
  møtemappe_id DROP NOT NULL,
...
-- TODO: This trigger should be removed when the legacy import is killed
CREATE
OR REPLACE FUNCTION enrich_legacy_moetesaksregistrering() RETURNS TRIGGER AS $ $ BEGIN IF NEW._external_id IS NULL
AND NEW.møtesaksregistrering_iri IS NOT NULL
AND NEW.møtesaksregistrering_iri != NEW._id THEN NEW._external_id := NEW.møtesaksregistrering_iri;
...
CREATE TRIGGER enrich_legacy_moetesaksregistrering_trigger BEFORE
INSERT
  OR
UPDATE
  ON møtesaksregistrering FOR EACH ROW EXECUTE FUNCTION enrich_legacy_moetesaksregistrering();

-- look up journalenhet__id
DROP TRIGGER IF EXISTS enrich_legacy_moetesaksregistrering_journalenhet_trigger ON møtesaksregistrering;

CREATE TRIGGER enrich_legacy_moetesaksregistrering_journalenhet_trigger BEFORE
INSERT
  OR
UPDATE
  ON møtesaksregistrering FOR EACH ROW EXECUTE FUNCTION enrich_legacy_journalenhet();
...
/* Identifikator */
CREATE TABLE IF NOT EXISTS identifikator(_id TEXT DEFAULT einnsyn_id('ide'));

ALTER TABLE
  IF EXISTS identifikator
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('ide'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS navn TEXT,
ADD
  COLUMN IF NOT EXISTS identifikator TEXT,
ADD
  COLUMN IF NOT EXISTS initialer TEXT,
ADD
  COLUMN IF NOT EXISTS epostadresse TEXT,
  /* This is a legacy field, but the object should inherit from ArkivBase: */
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT;
...
/* Moetedeltaker */
CREATE TABLE IF NOT EXISTS moetedeltaker(_id TEXT DEFAULT einnsyn_id('md'));

ALTER TABLE
  IF EXISTS moetedeltaker
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('md'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS moetedeltaker_navn TEXT,
ADD
  COLUMN IF NOT EXISTS moetedeltaker_funksjon TEXT,
  /* This is a legacy field, but the object should inherit from ArkivBase: */
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT;
...
/* Votering */
CREATE TABLE IF NOT EXISTS votering(_id TEXT DEFAULT einnsyn_id('vot'));

ALTER TABLE
  IF EXISTS votering
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('vot'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS moetedeltaker__id TEXT,
ADD
  COLUMN IF NOT EXISTS representerer__id TEXT,
ADD
  COLUMN IF NOT EXISTS vedtak__id TEXT,
ADD
  COLUMN IF NOT EXISTS stemme TEXT,
  /* This is a legacy field, but the object should inherit from ArkivBase: */
ADD
  COLUMN IF NOT EXISTS virksomhet_iri TEXT;

SELECT
  add_foreign_key_if_not_exists('votering', 'journalenhet__id', 'enhet', '_id');
...
SELECT
  add_foreign_key_if_not_exists('votering', 'vedtak__id', 'vedtak', '_id');
...
ALTER TABLE
  IF EXISTS bruker
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('bru'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalenhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS language TEXT DEFAULT 'nb';

SELECT
  add_foreign_key_if_not_exists('bruker', 'journalenhet__id', 'enhet', '_id');
...
/* Innsynskrav */
ALTER TABLE
  IF EXISTS innsynskrav
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('ik'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS locked BOOLEAN DEFAULT false,
ADD
  COLUMN IF NOT EXISTS bruker__id TEXT,
ADD
  COLUMN IF NOT EXISTS innsynskrav_version INT DEFAULT 0;

-- Used to differentiate between innsynskrav from the old/new api
SELECT
  add_foreign_key_if_not_exists('innsynskrav', 'bruker__id', 'bruker', '_id');
...
UPDATE
  innsynskrav
SET
  bruker__id = bruker._id
FROM
  bruker
WHERE
  innsynskrav.bruker_iri = bruker._external_id;

/* TODO: Trigger for incoming data? bruker__id */
/* InnsynskravDel */
ALTER TABLE
  IF EXISTS innsynskrav_del
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('ikd'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS journalpost__id TEXT,
ADD
  COLUMN IF NOT EXISTS enhet__id TEXT,
ADD
  COLUMN IF NOT EXISTS sent TIMESTAMPTZ,
ADD
  COLUMN IF NOT EXISTS retry_count INT NOT NULL DEFAULT 0,
ADD
  COLUMN IF NOT EXISTS retry_timestamp TIMESTAMPTZ,
ADD
  COLUMN IF NOT EXISTS lock_version BIGINT NOT NULL DEFAULT 1;
...
SELECT
  add_foreign_key_if_not_exists('innsynskrav_del', 'enhet__id', 'enhet', '_id');
...
CREATE INDEX IF NOT EXISTS innsynskrav_del_retries ON innsynskrav_del(
  sent,
  innsynskrav_id,
  retry_timestamp,
  retry_count
...
/* ApiKey */
CREATE TABLE IF NOT EXISTS api_key(_id TEXT DEFAULT einnsyn_id('key'));

ALTER TABLE
  IF EXISTS api_key
ADD
  COLUMN IF NOT EXISTS _id TEXT DEFAULT einnsyn_id('key'),
ADD
  COLUMN IF NOT EXISTS _external_id TEXT,
ADD
  COLUMN IF NOT EXISTS _created TIMESTAMPTZ DEFAULT now(),
...
ADD
  COLUMN IF NOT EXISTS lock_version BIGINT NOT NULL DEFAULT 1,
ADD
  COLUMN IF NOT EXISTS name TEXT,
ADD
  COLUMN IF NOT EXISTS secret TEXT,
ADD
  COLUMN IF NOT EXISTS enhet__id TEXT;

SELECT
  add_foreign_key_if_not_exists('api_key', 'enhet__id', 'enhet', '_id');
...
UPDATE
  enhet
SET
  parent_id = rootEnhetId
WHERE
  parent_id IS NULL
  AND _id != rootEnhet_Id;

/* Insert API key for root enhet */
INSERT INTO
  api_key (name, secret, enhet__id)
VALUES
  (
    'Root API key',
